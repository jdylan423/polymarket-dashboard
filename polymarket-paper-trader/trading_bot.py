...
Mean-reversion strategy on crypto prediction markets
"""
...
class PolymarketTradingBot:
    ...
    def run(self):
        """Main trading loop"""
        logger.info("=" * 80)
        logger.info("POLYMARKET PAPER TRADING BOT STARTED")
        logger.info("=" * 80)
    ...
    def _execute_trading_cycle(self):
        ...
        for market_id, asset_name in markets_config.items():
            ...
            trade = self._create_trade(market_id, prices, signal)
            self.journal.add_trade(trade)
            
            logger.info(f"✅ Trade recorded: {trade['market_id']}")
            self._print_stats()
    
    def _check_market_conditions(self, market_id: str, prices: Dict) -> bool:
        """Check if market meets minimum conditions for trading"""
        ...
        if prices["volume24hr"] < params["min_24h_volume"]:
            logger.debug(f"Insufficient 24h volume: ${prices['volume24hr']:.0f}")
            return False
    ...
    def _generate_signal(self, market_id: str, prices: Dict, asset_name: str) -> Optional[Dict]:
        ...
        # Get outcome probabilities (YES vs NO)
        ...
        yes_price = outcome_prices[0]  # Probability of YES
        no_price = outcome_prices[1]   # Probability of NO
        
        logger.info(f"   YES: {yes_price:.4f} | NO: {no_price:.4f}")
        
        # Mean reversion logic
        # If YES price drops below oversold threshold, it's extreme
        if yes_price < params["oversold_threshold"]:
            return {
                "action": "buy",
                "outcome": "Yes",
                "entry_price": yes_price,
                "reason": f"Oversold signal: YES @ {yes_price:.4f} < {params['oversold_threshold']}",
                "side": "buy"
            }
        
        # If YES price rises above overbought threshold, it's extreme
        elif yes_price > params["overbought_threshold"]:
            return {
                "action": "sell",
                "outcome": "Yes",
                "entry_price": yes_price,
                "reason": f"Overbought signal: YES @ {yes_price:.4f} > {params['overbought_threshold']}",
                "side": "sell"
            }
        
        return None
    
    def _create_trade(self, market_id: str, prices: Dict, signal: Dict) -> Dict:
        ...
        return {
            ...
            "conditionId": prices.get("conditionId"),
